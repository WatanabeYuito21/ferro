...
4. **新着メールのインデックス投入は非同期バックグラウンドタスク** — UIをブロックしない。
   ...
   5分間隔（`BACKGROUND_SYNC_INTERVAL`。設定画面はまだ無くハードコード）で全アカウントを
   自動同期し、結果を`background-sync`イベントでフロントエンドに通知する
   （`use_tls=false`のアカウントは自動同期の対象外＝手動Syncのみ。ユーザーが明示操作した
   わけではない自動実行で平文接続を暗黙に選ばないため）。CLIには同等のスケジューラはない
   （`ferro sync`を`cron`/タスクスケジューラ等の外部から定期実行する運用を想定）。
